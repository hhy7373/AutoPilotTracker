...
10. `supabase/migrations/202609100002_v041_public_catalog_views.sql`
...
v0.4.1 的两条迁移必须在 v0.3.2 目录状态迁移之后执行。第一条增加 `vehicle_brand`、来源核验补充字段、`system_vehicle_compatibility` 和管理员证据读取策略，并收紧系统、版本、车型的公开 RLS；第二条重建公开版本统计视图。两条脚本都要在 Supabase SQL Editor 中按顺序运行，不能只部署前端或 API 来替代数据库迁移。
